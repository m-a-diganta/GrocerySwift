...
// If no valid subcategory ID, redirect to homepage
if ($subcategory_id <= 0) {
...
// Get subcategory info and its parent category
$subcategory = null;
$parent_category = null;

// Function to get subcategory by ID with parent category info
function getSubcategoryWithParent($subcategory_id) {
    global $conn;
    $subcategory_id = (int)$subcategory_id;
    
    $sql = "SELECT s.*, c.category_id, c.category_name, c.category_image 
...
// Get subcategory with parent info
$subcategory_with_parent = getSubcategoryWithParent($subcategory_id);

// If subcategory not found, redirect to homepage
if (!$subcategory_with_parent) {
    header('Location: index.php');
    exit;
}

// Extract subcategory and parent category info
$subcategory = [
    'subcategory_id' => $subcategory_with_parent['subcategory_id'],
    'subcategory_name' => $subcategory_with_parent['subcategory_name'],
    'subcategory_image' => $subcategory_with_parent['subcategory_image'] ?? null
];

$parent_category = [
    'category_id' => $subcategory_with_parent['category_id'],
    'category_name' => $subcategory_with_parent['category_name'],
    'category_image' => $subcategory_with_parent['category_image']
];
...
// Get other subcategories in the same category
function getOtherSubcategoriesInCategory($category_id, $current_subcategory_id) {
    global $conn;
    $category_id = (int)$category_id;
    $current_subcategory_id = (int)$current_subcategory_id;
    
    $sql = "SELECT * FROM subcategories 
            WHERE category_id = $category_id 
            AND subcategory_id != $current_subcategory_id
...
$other_subcategories = getOtherSubcategoriesInCategory($parent_category['category_id'], $subcategory_id);
?>
...
<head>
    ...
    <title><?php echo htmlspecialchars($subcategory['subcategory_name']); ?> - GrocerySwift</title>
...
<body>
                                    ...
                                    <?php if (!empty($subcategories)): ?>
                                        <div class="subcategory-content">
                                            <?php foreach ($subcategories as $subcat): ?>
                                                <a href="subcategory.php?id=<?php echo $subcat['subcategory_id']; ?>">
    ...
    <div class="container">
        ...
        <div class="breadcrumb" style="margin-top: 20px; margin-bottom: 10px; font-size: 14px; font-weight: bold;">
            ...
            <a href="category.php?id=<?php echo $parent_category['category_id']; ?>"><?php echo htmlspecialchars($parent_category['category_name']); ?></a> &gt;
            <span><?php echo htmlspecialchars($subcategory['subcategory_name']); ?></span>
        </div>
        
        <section class="subcategory-header" style="background-color:#02383c; padding: 10px; border-radius: 10px; text-align: center; color: white; margin-bottom: 20px; margin-top: 20px;">
            <div class="subcategory-banner">
                <h1><?php echo htmlspecialchars($subcategory['subcategory_name']); ?></h1>
            </div>
        </section>
        
        <section class="subcategory-products">
            <?php if (empty($products)): ?>
                <p class="no-products">No products found in this subcategory.</p>
            <?php else: ?>
                    ...
                    <?php foreach ($products as $product): ?>
                        <div class="product-card">
                            ...
                            <div class="product-info">
                                ...
                                <form action="subcategory.php?id=<?php echo $subcategory_id; ?>" method="POST">
        ...
        <section class="other-categories">
            ...
            <div class="category-grid" style="margin-top: 20px;">
                <?php foreach ($categories as $category): ?>
                    <?php if ($category['category_id'] != $parent_category['category_id']): ?>
